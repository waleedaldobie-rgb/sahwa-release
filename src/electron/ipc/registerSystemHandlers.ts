...
export function registerSystemHandlers(deps: SystemHandlersDeps): void {
  ...
  const printingService = new PrintingService(
    () => dbManager.getUserPreferences().invoicePrinterName,
    (name) => { dbManager.updateUserPreferences({ invoicePrinterName: name }); },
  );
  ...
  safeIpcHandle(ipcMain, 'printers:list', async (event) => printingService.listPrinters(event.sender));
  safeIpcHandle(ipcMain, 'printers:printWithDialog', async (event) => printingService.printWithDialog(event.sender));
  safeIpcHandle(ipcMain, 'printers:printDirect', async (event, options?: { deviceName?: string; pageSize?: '15x21'; allowDialogFallback?: boolean }) => (
    printingService.printDirect(event.sender, options)
  ));
  safeIpcHandle(ipcMain, 'printers:savePDF', async (event) => printingService.saveCurrentPageAsPdf(event.sender));
